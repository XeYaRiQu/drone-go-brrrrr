...
[![Humble](https://img.shields.io/endpoint?url=https://gist.githubusercontent.com/NovoG93/589e4b4dc8d92861e4b92defff6d56c0/raw/_humble_build.json)](https://github.com/NovoG93/drone/actions/workflows/build.yml) [![Rolling](https://img.shields.io/endpoint?url=https://gist.githubusercontent.com/NovoG93/589e4b4dc8d92861e4b92defff6d56c0/raw/_rolling_build.json)](https://github.com/NovoG93/drone/actions/workflows/build.yml)

drone is a quadrotor simulation program forked from [tum_simulator](http://wiki.ros.org/tum_simulator), developed using ROS + Gazebo.
...
```
cd ~/git && git clone git@github.com:NovoG93/drone.git -b ros2
cd ~/ros2_ws/src && ln -s ~/git/drone
cd .. && rosdep install -r -y --from-paths src --ignore-src --rosdistro $ROS_DISTRO && colcon build --packages-select-regex sjtu*
```

To use the playground.world file (as depicted below) make sure to install the common gazebo models, for more see the [Readme in drone_description](./drone_description/README.md).
...
2. Connect to docker container to takeoff / land drone:
   1. `docker container exec -it drone 'ros2 topic pub /drone/takeoff std_msgs/msg/Empty {} --once'`
   1. `docker container exec -it drone 'ros2 topic pub /drone/land std_msgs/msg/Empty {} --once'`
...
The parameters of the PID controller that controls the drone and the motion noise can be changed by adapting the file [drone.urdf.xacro](drone_description/urdf/drone.urdf.xacro#L51-L80):
...
- Change of ROS namespace not working automatically for plugin_drone when changed in [spawn_drone.py](./drone_bringup/drone_bringup/spawn_drone.py)
